...
def createinstancee():
    if request.method == 'POST':
        ...
        createscript = f"gcloud compute instances create instance-8 --project=silent-card-399405 --zone=us-west3-a --machine-type=e2-medium --access-token-file=token.txt --network-interface=network-tier=PREMIUM,private-network-ip=10.180.6.100,subnet=default --metadata-from-file startup-script=my_startup_script.sh --service-account=87528303241-compute@developer.gserviceaccount.com --create-disk=auto-delete=yes,boot=yes,device-name=instance-1,image=projects/ubuntu-os-cloud/global/images/ubuntu-2004-focal-v20230907,mode=rw,size=10,type=projects/silent-card-399405/zones/us-west3-a/diskTypes/pd-balanced --no-shielded-secure-boot --shielded-vtpm --shielded-integrity-monitoring --labels=goog-ec-src=vm_add-gcloud --reservation-affinity=any"
        # print(createscript)
        return createscript
